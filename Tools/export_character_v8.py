"""Export reviewed V8 mesh/actions plus baked undercoat color; archives prior runtime albedo."""
...
if '--bake-only' in sys.argv:
 print('V8_UNDERCOAT_BAKE_READY');sys.exit(0)
...
bpy.ops.export_scene.fbx(filepath=str(root/'Art/Exports/Jimothy-v8.fbx'),use_selection=True,object_types={'MESH','ARMATURE'},add_leaf_bones=False,axis_forward='-Z',axis_up='Y',bake_anim=True,bake_anim_step=.125,bake_anim_simplify_factor=0,bake_anim_use_all_actions=True,bake_anim_use_nla_strips=False,path_mode='AUTO',colors_type='LINEAR')
print('CHARACTER_V8_EXPORTED_BAKED')
