...
def warp(p):
 x,y,z=p; t=max(0,min(1,(z-.8)/.8));return Vector((x*(1-.04*t),y*(1-.09*t),z+.075*t*max(0,1-abs(y)/.85)))
...
random.seed(501);skin.data.calc_loop_triangles();ts=list(skin.data.loop_triangles);cdf=[];area=0
for t in ts:
...
for k in range(8200):
 ...
 length=random.uniform(.045,.125)*( .38 if face else (.60 if pos.z<1 else 1))
 width=random.uniform(.018,.034)*( .55 if face else 1)
 ...
 tangent.normalize();side=n.cross(tangent).normalized();direction=(tangent*.72+n*.46).normalized()
 ...
 grizzle=random.random(); gain=1.5 if grizzle<.70 else 3.8
 col=Vector(tuple(min(.65,max(.02,c*gain*2.5)) for c in col))
 wg={}
 ...
 for j in range(4):
  ...
  for sign in [-1,1]:
   verts.append(center+side*width*.5*sign*(1-.8*f));uv.append(((sign+1)/2,f));colors.append((*tuple(col*(.72+.40*f)),1));weights.append(wg)
...
bpy.ops.wm.save_as_mainfile(filepath=str(root/'Art/Blender/Jimothy-v5.blend'),compress=True)
...
bpy.ops.export_scene.fbx(filepath=str(root/'Unity/Assets/Jimothy/Resources/Jimothy.fbx'),use_selection=True,object_types={'MESH','ARMATURE'},add_leaf_bones=False,axis_forward='-Z',axis_up='Y',bake_anim=True,bake_anim_use_all_actions=True,bake_anim_use_nla_strips=False,path_mode='AUTO')
(root/'Documentation/character-v5.json').write_text(json.dumps({'fur_cards':len(faces)//3,'fur_triangles':len(faces)*2,'base_triangles':sum(len(p.vertices)-2 for p in skin.data.polygons),'source':'Jimothy-v5.blend','gait_actions':[a.name for a in bpy.data.actions]},indent=2))
print('CHARACTER_V5_EXPORTED')
