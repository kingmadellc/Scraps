"""Read-only round trip of exported V8 FBX; independent of Unity rendering."""
import bpy,json,math
from pathlib import Path
root=Path(__file__).resolve().parents[1];bpy.ops.wm.read_factory_settings(use_empty=True);bpy.ops.import_scene.fbx(filepath=str(root/'Art/Exports/Jimothy-v8.fbx'),colors_type='LINEAR')
...
(root/'Documentation/character-v8-fbx-roundtrip.json').write_text(json.dumps(report,indent=2));print('V8_FBX_ROUNDTRIP_PASS',json.dumps(report))
