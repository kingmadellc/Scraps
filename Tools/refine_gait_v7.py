...
BODY=((0.,-.28,.085),(.13,-.34,.125),(.32,-.29,.05),(.51,-.24,-.035),(.68,-.20,-.055),(.84,-.24,.025),(1.,-.28,.085))
def body_pose(t):
...
def refine(rig):
    ...
    report={'version':7,'frames':FRAMES,'duration_seconds':duration,'nominal_armature_speed':NOMINAL,'nominal_world_speed_at_scale_022':NOMINAL*.22,'max_solver_error':0.,'max_reach_clamp':0.,'max_joint_gap':0.,'samples':0,'source':'Kiana Hall footage, Seattle Times RqlQYJUz-j8 opening run; see GAIT-STUDY-V7.md'}
    previous={}
    for sample in range((FRAMES-1)*8+1):
        ...
        for i,n in enumerate(LEGS):
            ...
            target.x+=(-1 if n.endswith('_L') else 1)*z*(.13 if i<2 else .20)
    ...
    action['loop']=True;action['gait_v7']=True;action['nominal_forward_speed_armature_units_per_second']=NOMINAL
