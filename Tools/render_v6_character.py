...
for name,loc in [('rear',(0,5,3.4)),('side',(-5,.2,2.5)),('front',(-3,-4,2.7))]:
 ...
 s.render.filepath=str(root/'Art/Renders'/('jimothy-v6-'+name+'.png'));bpy.ops.render.render(write_still=True)
s.render.resolution_x=480;s.render.resolution_y=480;s.cycles.samples=6
rig.animation_data.action=bpy.data.actions['Jump']
for f in [1,11,30,47,51,61]:
 s.frame_set(f);s.render.filepath=str(root/'Art/Renders'/('jump-v6-%02d.png'%f));bpy.ops.render.render(write_still=True)
