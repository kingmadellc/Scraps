...
[InitializeOnLoad] public static class DenPolishArtAudit {
 const string Pending="Jimothy.DenPolishArtAudit";const int Width=1280,Height=720,Count=4;
 ...
 static void Tick(){if(!SessionState.GetBool(Pending,false)||!EditorApplication.isPlaying)return;try{
  if(shot<0){game=UnityEngine.Object.FindFirstObjectByType<GameSession>();if(!game)return;game.SuppressSaving=true;game.NewGame();game.Player.Teleport(ClosingTimeWorld.DenInterior);game.Data.coins=2000;foreach(var id in game.Items.Keys){game.Data.pantry.Add(id);game.Data.bankedDiscoveries.Add(id);}foreach(var id in DenFurnishings.Catalog.Keys)game.BuyDecor(id,0);game.Player.FaceDirection(0);game.Player.RecenterCamera();game.Player.enabled=false;foreach(var canvas in game.GetComponentsInChildren<Canvas>())canvas.enabled=false;
   previousScale=Time.timeScale;camera=Camera.main;camera.fieldOfView=58;camera.aspect=16f/9;target=new RenderTexture(Width,Height,24,RenderTextureFormat.DefaultHDR){name="Roof motion evidence",antiAliasing=1};target.Create();camera.targetTexture=target;folder=Path.GetFullPath("../PlaytestCaptures/den-polish-v045");Directory.CreateDirectory(folder);sheet=new Texture2D(1920,270,TextureFormat.RGB24,false);report=new Report{savingSuppressed=game.SuppressSaving};shot=0;Pose();return;}
 ...
 }catch(Exception e){Debug.LogException(e);folder=Path.GetFullPath("../PlaytestCaptures/den-polish-v045");Directory.CreateDirectory(folder);File.WriteAllText(Path.Combine(folder,"error.txt"),e.ToString());Finish(1);}}
 static void Pose(){
  ...
  else if(shot==2){game.Player.Teleport(ClosingTimeWorld.DenEntrance);game.Player.FaceDirection(0);game.Player.RecenterCamera();for(int i=0;i<60;i++)game.Player.UpdateFollowCamera(1f/60);aim=game.Player.transform.position+Vector3.up*.3f;}
  if(shot==3){camera.transform.position=new Vector3(-17.9f,-1.25f,60.6f);aim=new Vector3(-18.05f,-1.7f,62.45f);camera.transform.LookAt(aim);}
