...
public static class GuidedStairAudit {
 ...
 public static void Run(){
  ...
  var report=new Report{utc=DateTime.UtcNow.ToString("O"),method="Ten actual authored roof routes: jump requests allowed only for the first access tread, then directional movement and supported auto-mantle; actual CC at60Hz."};
  try {
   ...
   for(int r=0;r<paths.Count;r++){
    ...
    for(int p=0;p<points.Length;p++){
     ...
     for(int frame=0;frame<240;frame++){
      ...
      bool request=p==0 && needRise && motor.Grounded && frame*Dt-lastJump>.35f;
    ...
    Debug.Log("GUIDED_STAIR_ROUTE "+r+" reached="+route.reached+" failedStep="+route.failedStep);
   }
   ...
   string path=Path.GetFullPath(Path.Combine(Application.dataPath,"../Logs/guided-stair-audit.json"));Directory.CreateDirectory(Path.GetDirectoryName(path));File.WriteAllText(path,JsonUtility.ToJson(report,true));
   Debug.Log("GUIDED_STAIR_AUDIT "+(report.passed?"PASS":"FAIL")+" "+path);
  }
