...
[InitializeOnLoad] public static class CollectorArtAudit {
 const string Key="Jimothy.CollectorArtAudit";
 static readonly string[] Pages={"den","locked","ready","claimed"};
 ...
 static CollectorArtAudit(){EditorApplication.playModeStateChanged+=s=>{if(s==PlayModeStateChange.EnteredPlayMode&&SessionState.GetBool(Key,false)){shot=-1;EditorApplication.update+=Tick;}};}
 ...
 static void Pose(){
  ...
  game.Resume();switch(Pages[shot%Pages.Length]){case "locked":game.Data.bankedRuns=0;game.Data.collectorTrades=0;game.Data.collectorLastRun=0;game.UI.ShowCollector();break;case "ready":game.Data.bankedRuns=1;game.Data.collectorLastRun=0;game.Data.pantry.Clear();game.Data.pantry.AddRange(new[]{"item_000","item_002"});game.UI.ShowCollector();break;case "claimed":game.Data.collectorLastRun=game.Data.bankedRuns;game.Data.collectorTrades=3;game.UI.ShowCollector();break;case "bare":case "bandana":case "crown":case "roadie":case "boat":case "inspector":game.UI.ShowWardrobe(Array.FindIndex(WardrobeRules.Looks,x=>x.id==Pages[shot%Pages.Length]));break;case "title":game.Menu();break;case "pause":game.TogglePause();break;case "controls":game.TogglePause();Invoke("ShowTouchHelp");break;case "settings":game.TogglePause();Invoke("Settings",true);break;case "outing":game.UI.ShowNightBoard();break;case "rules":game.UI.ShowNightBoard();Invoke("ShowOutingRules");break;case "collections":game.UI.ShowNightBoard();Invoke("ShowCollections");break;case "den":game.UI.OpenDenForPlaytest();break;case "hud":game.UI.ShowHUD();break;}
 ...
 static void Tick(){if(!EditorApplication.isPlaying)return;try{
  if(shot<0){game=UnityEngine.Object.FindFirstObjectByType<GameSession>();if(!game)return;GameUI.ForceMobileLayout=true;game.SuppressSaving=true;game.NewGame();game.Data.bankedDiscoveries.AddRange(new[]{"item_176","item_128","item_000"});game.Data.bankedRuns=4;game.Player.enabled=false;game.Player.UpdateFollowCamera(.1f);camera=Camera.main;oldTime=Time.timeScale;Time.timeScale=0;folder=Path.GetFullPath("../PlaytestCaptures/collector-070");Directory.CreateDirectory(folder);report=new Report();shot=0;Pose();return;}
