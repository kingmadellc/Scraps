...
[InitializeOnLoad] public static class FurVisualAudit {
 ...
 if(shot<0){game=UnityEngine.Object.FindFirstObjectByType<GameSession>();if(!game)return;game.SuppressSaving=true;game.NewGame();game.StartNight();game.Player.Teleport(ClosingTimeWorld.StreetStart);game.Player.FaceDirection(180);game.TogglePause();game.Player.enabled=false;origin=game.Player.transform.position;foreach(var c in game.GetComponentsInChildren<Canvas>())c.enabled=false;foreach(var r in game.Player.GetComponentsInChildren<Renderer>()){foreach(var m in r.sharedMaterials){if(m.shader.name=="Jimothy/RootedFur")fur=r;if(m.name.Contains("painted coat"))skin=m;}}
 if(!fur||!skin)throw new Exception("Missing coat renderers");cam=Camera.main;cam.fieldOfView=50;cam.aspect=16f/9;cam.nearClipPlane=.03f;target=new RenderTexture(1280,720,24,RenderTextureFormat.DefaultHDR){antiAliasing=2};target.Create();cam.targetTexture=target;folder=Path.GetFullPath("../PlaytestCaptures/fur-v10-diagnostic");Directory.CreateDirectory(folder);notes="Saving suppressed: "+game.SuppressSaving+"\n0 full close; 1 no geometric fur; 2 full fur no base normal; 3 full play distance; 4 no fur play distance; 5 full fur/no normal play distance; 6–11 run, 12–14 walk, 15–17 stop; actual locomotion with spring logged at six decimals. Target requests2xMSAA; no supersampling.\n";shot=0;Pose();return;}
 if(EditorApplication.timeSinceStartup<ready)return;
 if(shot>=6&&shot<18){cam.transform.position=game.Player.transform.position+new Vector3(.6f,.65f,1.8f);cam.transform.LookAt(game.Player.transform.position+Vector3.up*.35f);}
 ...
 File.WriteAllText(Path.Combine(folder,"evidence.txt"),notes);shot++;if(shot==22){File.WriteAllText(Path.Combine(folder,"evidence.txt"),notes);Finish(0);return;}Pose();
 }catch(Exception e){Debug.LogException(e);Finish(1);}}
 static void Pose(){fur.enabled=shot%3!=1||shot>=6;skin.SetFloat("_BumpScale",shot%3==2?0:.65f);cam.transform.position=origin+new Vector3(.2f,shot<3?.5f:.8f,shot<3?.9f:2.8f);cam.transform.LookAt(origin+Vector3.up*.35f);if(shot==12)game.Player.touchMove=new Vector2(0,.35f);if(shot==15)game.Player.touchMove=Vector2.zero;if(shot==6){skin.SetFloat("_BumpScale",.65f);game.TogglePause();game.Player.enabled=true;game.Player.touchMove=new Vector2(0,1);}if(shot==18){game.Player.touchMove=Vector2.zero;game.TogglePause();game.Player.enabled=false;game.Player.Teleport(origin);game.Player.FaceDirection(180);}
 if(shot>=18){var p=game.Player.transform.position;cam.transform.position=p+(shot==18?new Vector3(.95f,.52f,.12f):shot==19?new Vector3(-.5f,.48f,-.95f):new Vector3(.35f,.64f,1.1f));cam.transform.LookAt(p+Vector3.up*.34f);}
 if(shot==21){game.Player.Teleport(ClosingTimeWorld.DenInterior);game.Player.FaceDirection(0);var p=game.Player.transform.position;cam.transform.position=p+new Vector3(.5f,.53f,1.0f);cam.transform.LookAt(p+Vector3.up*.34f);}
 ready=EditorApplication.timeSinceStartup+(shot>=6&&shot<18?.13:.65);}
