...
[InitializeOnLoad] public static class UIDesignAudit {
 const string Key="Jimothy.UIDesignAudit";
 static readonly string[] Pages={"title","pause","controls","settings","outing","rules","collections","den","hud"};
 ...
 static UIDesignAudit(){EditorApplication.playModeStateChanged+=s=>{if(s==PlayModeStateChange.EnteredPlayMode&&SessionState.GetBool(Key,false)){shot=-1;EditorApplication.update+=Tick;}};}
 ...
 static void Pose(){
  ...
  game.Resume();switch(Pages[shot%Pages.Length]){case "title":game.Menu();break;case "pause":game.TogglePause();break;case "controls":game.TogglePause();Invoke("ShowTouchHelp");break;case "settings":game.TogglePause();Invoke("Settings",true);break;case "outing":game.UI.ShowNightBoard();break;case "rules":game.UI.ShowNightBoard();Invoke("ShowOutingRules");break;case "collections":game.UI.ShowNightBoard();Invoke("ShowCollections");break;case "den":game.UI.OpenDenForPlaytest();break;case "hud":game.UI.ShowHUD();break;}
 ...
 static void Tick(){if(!EditorApplication.isPlaying)return;try{
  if(shot<0){game=UnityEngine.Object.FindFirstObjectByType<GameSession>();if(!game)return;GameUI.ForceMobileLayout=true;game.SuppressSaving=true;game.NewGame();game.Player.enabled=false;game.Player.UpdateFollowCamera(.1f);camera=Camera.main;oldTime=Time.timeScale;Time.timeScale=0;folder=Path.GetFullPath("../PlaytestCaptures/ui-design-050");Directory.CreateDirectory(folder);report=new Report();shot=0;Pose();return;}
