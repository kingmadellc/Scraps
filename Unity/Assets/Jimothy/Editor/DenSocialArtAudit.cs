...
[InitializeOnLoad] public static class DenSocialArtAudit {
 const string Pending="Jimothy.DenSocialArtAudit";const int Width=1280,Height=720,Count=6;
 ...
 static DenSocialArtAudit(){EditorApplication.playModeStateChanged+=OnMode;if(SessionState.GetBool(Pending,false))EditorApplication.update+=Tick;}
 [MenuItem("Scraps/Capture Den social 070 evidence")]
 ...
 }catch(Exception e){Debug.LogException(e);folder=Path.GetFullPath("../PlaytestCaptures/den-social-070");Directory.CreateDirectory(folder);File.WriteAllText(Path.Combine(folder,"error.txt"),e.ToString());Finish(1);}}
 static void Pose(){
  if(shot==3){game.Data.collectorTrades=3;game.Data.bankedRuns=4;game.Data.coins=2000;foreach(var id in game.Items.Keys){game.Data.pantry.Add(id);game.Data.bankedDiscoveries.Add(id);}foreach(var id in DenFurnishings.Catalog.Keys)game.BuyDecor(id,0);}
  int view=shot%3;
  ...
  else if(view==1){camera.transform.position=new Vector3(-11.6f,-1.4f,56.6f);aim=new Vector3(-10.35f,-1.75f,58.4f);camera.transform.LookAt(aim);}
  else{camera.transform.position=new Vector3(-17.4f,.35f,58.7f);aim=new Vector3(-19.05f,-.05f,60);camera.transform.LookAt(aim);}
