...
public partial class GameUI {
 ...
 void BuildTouchControls(){
  ...
  var jump=TouchAction("Jump and trick","jump",new(1,0),new(-116,120),148,()=>{});var gesture=jump.gameObject.AddComponent<TouchTrickGesture>();gesture.motor=game.Player;gesture.fill=jump.GetComponent<Image>();gesture.feedback=TouchCaption(jump.transform,"TAP JUMP · SWIPE TO TRICK",-102,290);gesture.feedback.rectTransform.anchoredPosition+=Vector2.left*38;
  ...
  var hint=TouchCaption(page.transform,"LEFT THUMB MOVE · RIGHT SIDE LOOK",0,500);hint.rectTransform.anchorMin=hint.rectTransform.anchorMax=new(.5f,0);hint.rectTransform.anchoredPosition=new(0,30);hint.color=new(.75f,.83f,.72f,.8f);
 }
 void UpdateTouchControls(){if(!MobileLayout||!game.Player)return;if(touchContext){bool available=game.AtHome||game.NearbyLoot;touchContext.gameObject.SetActive(available);if(available){touchContextLabel.text=game.AtHome?"STASH":"SEARCH";var glyph=touchContext.GetComponentInChildren<TouchGlyph>();string icon=game.AtHome?"stash":"search";if(glyph.symbol!=icon){glyph.symbol=icon;glyph.SetVerticesDirty();}}}if(touchFood&&game.Data!=null)touchFood.gameObject.SetActive((game.Data.health<99||game.Data.hunger<99)&&(game.Data.bag.Exists(id=>game.Items.TryGetValue(id,out var item)&&item.category=="food")||game.AtHome&&game.Data.pantry.Exists(id=>game.Items.TryGetValue(id,out var item)&&item.category=="food")));}
 void ShowTouchHelp(){Clear(green);Heading(page.transform,"SMALL PAWS. BIG AIR.",48,cream,new(.10f,.77f),new(.92f,.89f)).font=TitleDisplay;
  Placed(page.transform,"LEFT THUMB  Move in any direction. Small drags walk; longer drags run.\nRIGHT SIDE  Drag open space to steer the camera.\nJUMP DISC  Tap to jump. Swipe from the disc to add a trick.\nUP / DOWN  Frontflip / backflip.  LEFT / RIGHT  Barrel roll.\nLAND CLEAN  Complete the trick before touching down to earn style.",36,cream,new(.10f,.29f),new(.91f,.75f));
  ...
  Button(page.transform,"Back",new(.65f,.14f),new(.9f,.25f),ShowPause);
 }
}
}
