...
public sealed class MobileTouchSafety:MonoBehaviour {
 ...
 void Update(){if(!game||!GameUI.MobileLayout)return;bool portrait=Screen.height>Screen.width;if(portrait){StopInputs();if(game.Playing&&!game.Paused)game.TogglePause();}if(Screen.width!=lastWidth||Screen.height!=lastHeight){lastWidth=Screen.width;lastHeight=Screen.height;StopInputs();}
